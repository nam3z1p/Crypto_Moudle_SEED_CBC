...
// S-BOX
const static unsigned int SS0[256] =
    {
...
const static unsigned int SS1[256] =
    {
...
const static unsigned int SS2[256] =
    {
...
const static unsigned int SS3[256] =
    {
...
#define EndianChange(dwS)                          \
    ((ROTL((dwS), 8) & (unsigned int)0x00ff00ff) | \
     (ROTL((dwS), 24) & (unsigned int)0xff00ff00))
...
void KISA_SEED_Encrypt_Block_forCBC(unsigned int *in, unsigned int *out, KISA_SEED_KEY *ks)
{
    ...
    unsigned int T0, T1;            // Temporary variables for round function F
    unsigned int *K = ks->key_data; // Pointer of round keys
...
int SEED_CBC_init(OUT KISA_SEED_INFO *pInfo, IN KISA_ENC_DEC enc, IN unsigned char *pbszUserKey, IN unsigned char *pbszIV)
{
    ...
    A = ((unsigned int)pbszUserKey[3] << 24) | ((unsigned int)pbszUserKey[2] << 16) | ((unsigned int)pbszUserKey[1] << 8) | ((unsigned int)pbszUserKey[0]);
    B = ((unsigned int)pbszUserKey[7] << 24) | ((unsigned int)pbszUserKey[6] << 16) | ((unsigned int)pbszUserKey[5] << 8) | ((unsigned int)pbszUserKey[4]);
    C = ((unsigned int)pbszUserKey[11] << 24) | ((unsigned int)pbszUserKey[10] << 16) | ((unsigned int)pbszUserKey[9] << 8) | ((unsigned int)pbszUserKey[8]);
    D = ((unsigned int)pbszUserKey[15] << 24) | ((unsigned int)pbszUserKey[14] << 16) | ((unsigned int)pbszUserKey[13] << 8) | ((unsigned int)pbszUserKey[12]);
    ...
    *outLen = 0;
    ...
    if (KISA_ENCRYPT == pInfo->encrypt)
    {
        ...
        for (i = pInfo->buffer_length; i < BLOCK_SIZE_SEED; i++)
            ((unsigned char *)pInfo->cbc_buffer)[i] = (unsigned char)nPaddngLeng;
    ...
    {
        nPaddngLeng = ((unsigned char *)pInfo->cbc_last_block)[BLOCK_SIZE_SEED - 1];
...
int SEED_CBC_Decrypt(IN unsigned char *pbszUserKey, IN unsigned char *pbszIV, IN unsigned char *pbszCipherText, IN int nCipherTextLen, OUT unsigned char *result)
{
    KISA_SEED_INFO info;
    unsigned int *outbuf;
    unsigned int *data;
    unsigned char *newpbszCipherText;
    unsigned char *pbszPlainText;
    unsigned char *cdata;
    int outlen = 0;
    int nRetOutLeng = 0;
    int nPaddingLeng = 0;
...
#define Main_Title()                                                    \
    printf("######################################################\n"); \
    printf("##          Crypto Moudle SEED CBC _v0.1            ##\n"); \
    printf("##                                                  ##\n"); \
...
#define Menu()                                              \
    ...
    printf("\nex) %s -e[-d] test.bin \n", argv[0]);         \
    printf("\n[-e, -d] \n");                                \
    printf("  -e              Encrypt Seed File\n");        \
    printf("  -d              Decrypt Seed File\n");        \
...
typedef struct _SEED_FORMAT
{
    PBYTE pBuffer;
    PBYTE pSeedHeader;
    DWORD dwSeedHeaderSize;
    DWORD dwSeedTextSize;
    DWORD dwSizeCount;
    DWORD dwSizeLeft;
    DWORD dwSeedResultTextSize;
    DWORD dwExtraSize;
} SEEDFORMAT, *PSEEDFORMAT;

void Print_Row_Data(const PBYTE pstr, DWORD dwRange)
{
...
PFILEVIEW GetFileView(LPCTSTR szFileName)
{
    ...
    {
        printf("[!] It's not found the hFileView !!\n");
        return FALSE;
    }
...
PSEEDFORMAT Decrypt_Seed(PFILEVIEW pFileView, PBYTE pbszUserKey, PBYTE pbszIV)
{
    PSEEDFORMAT pSeedFormat = {0};
    DWORD i = 0;

    pSeedFormat = malloc(sizeof(SEEDFORMAT));
    memset(pSeedFormat, 0, sizeof(SEEDFORMAT));

    pSeedFormat->dwSeedHeaderSize = 0x20;
    pSeedFormat->dwSeedTextSize = pFileView->dwFileSize - pSeedFormat->dwSeedHeaderSize;
    pSeedFormat->dwSizeCount = pSeedFormat->dwSeedTextSize / 1024;
    pSeedFormat->dwSizeLeft = pSeedFormat->dwSeedTextSize % 1024;

    pSeedFormat->pSeedHeader = malloc(pSeedFormat->dwSeedHeaderSize);
    memset(pSeedFormat->pSeedHeader, 0, pSeedFormat->dwSeedHeaderSize);

    memcpy(pSeedFormat->pSeedHeader, pFileView->pBaseAddr, pSeedFormat->dwSeedHeaderSize);

    pSeedFormat->pBuffer = (PBYTE)VirtualAlloc(NULL, (pSeedFormat->dwSizeCount + 1) * 1023, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    if (pSeedFormat->pBuffer == NULL)
    {
        printf("[!] It's not found the VirtualAlloc pSeedFormat->pBuffer !!\n");
        return FALSE;
    }

    memset(pSeedFormat->pBuffer, 0, (pSeedFormat->dwSizeCount + 1) * 1023);

    for (i = 0; i < pSeedFormat->dwSizeCount; i++)
    {
        //int SEED_CBC_Decrypt(IN unsigned char *pbszUserKey, IN unsigned char *pbszIV, IN unsigned char *pbszCipherText, IN int nCipherTextLen, OUT unsigned char *result)

        SEED_CBC_Decrypt(pbszUserKey, pbszIV, &pFileView->pBaseAddr[(i * 1024) + pSeedFormat->dwSeedHeaderSize], 1024, &pSeedFormat->pBuffer[i * 1023]);
    }

    pSeedFormat->dwExtraSize = SEED_CBC_Decrypt(pbszUserKey, pbszIV, &pFileView->pBaseAddr[(i * 1024) + pSeedFormat->dwSeedHeaderSize], pSeedFormat->dwSizeLeft, &pSeedFormat->pBuffer[i * 1023]);

    pSeedFormat->dwSeedResultTextSize = (pSeedFormat->dwSizeCount * 1023) + (pSeedFormat->dwSizeLeft - pSeedFormat->dwExtraSize);

    if (DEBUG)
    {
        printf("==========================================\n");
        printf("[+] dwSeedTextSize : %d \n", pSeedFormat->dwSeedTextSize);
        printf("[+] dwSizeCount : %d \n", pSeedFormat->dwSizeCount);
        printf("[+] dwSizeLeft : %d \n", pSeedFormat->dwSizeLeft);
        printf("[+] dwExtraSize : %d \n", pSeedFormat->dwExtraSize);
        printf("[+] dwSeedResultTextSize : %d \n", pSeedFormat->dwSeedResultTextSize);

        printf("[+] Key : ");
        Print_Row_Data(pbszUserKey, 16);
        printf("[+] IV : ");
        Print_Row_Data(pbszIV, 16);
        printf("[+] Seed Header : ");
        Print_Row_Data(pSeedFormat->pSeedHeader, pSeedFormat->dwSeedHeaderSize);

        //printf("pBffuer : ");
        //Print_Row_Data(pSeedFormat->pBuffer, 0x40);
        printf("==========================================\n");
    }
...
PSEEDFORMAT Encrypt_Seed(PFILEVIEW pFileView, PBYTE pbszUserKey, PBYTE pbszIV)
{
    PSEEDFORMAT pSeedFormat = {0};
    DWORD i = 0;

    BYTE SeedHeader[] = {0x53, 0x31, 0x41, 0x31, 0xE5, 0x07, 0x04, 0x15, 0x0E, 0x38, 0x0E, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    // 61,62,63,64,65,66,67,68,69,70 <-> AE CD 10 FA B4 5C AD 5D 6B 05 24 A8 D9 8C A4 32

    pSeedFormat = malloc(sizeof(SEEDFORMAT));
    memset(pSeedFormat, 0, sizeof(SEEDFORMAT));

    pSeedFormat->dwSeedHeaderSize = 0x20;
    pSeedFormat->dwSeedTextSize = pFileView->dwFileSize;
    pSeedFormat->dwSizeCount = pSeedFormat->dwSeedTextSize / 1023;
    pSeedFormat->dwSizeLeft = pSeedFormat->dwSeedTextSize % 1023;

    pSeedFormat->pSeedHeader = malloc(pSeedFormat->dwSeedHeaderSize);
    memset(pSeedFormat->pSeedHeader, 0, pSeedFormat->dwSeedHeaderSize);

    pSeedFormat->pBuffer = (PBYTE)VirtualAlloc(NULL, ((pSeedFormat->dwSizeCount + 1) * 1040) + pSeedFormat->dwSeedHeaderSize, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    if (pSeedFormat->pBuffer == NULL)
    {
        printf("[!] It's not found the VirtualAlloc pSeedFormat->pBuffer !!\n");
        return FALSE;
    ...
    memset(pSeedFormat->pBuffer, 0, ((pSeedFormat->dwSizeCount + 1) * 1040) + pSeedFormat->dwSeedHeaderSize);

    memcpy(pSeedFormat->pSeedHeader, SeedHeader, pSeedFormat->dwSeedHeaderSize);
    memcpy(pSeedFormat->pBuffer, SeedHeader, pSeedFormat->dwSeedHeaderSize);

    for (i = 0; i < pSeedFormat->dwSizeCount; i++)
    {
        //int SEED_CBC_Encrypt(IN unsigned char *pbszUserKey, IN unsigned char *pbszIV, IN unsigned char *pbszPlainText, IN int nPlainTextLen, OUT unsigned char *pbszCipherText)

        SEED_CBC_Encrypt(pbszUserKey, pbszIV, &pFileView->pBaseAddr[i * 1023], 1023, &pSeedFormat->pBuffer[(i * 1024) + pSeedFormat->dwSeedHeaderSize]);
    }

    if (pSeedFormat->dwSizeLeft != 0)
    {
        pSeedFormat->dwExtraSize = SEED_CBC_Encrypt(pbszUserKey, pbszIV, &pFileView->pBaseAddr[i * 1023], pSeedFormat->dwSizeLeft, &pSeedFormat->pBuffer[(i * 1024) + pSeedFormat->dwSeedHeaderSize]);
    }
    else
    {
        pSeedFormat->dwExtraSize = 0x00;
    }

    pSeedFormat->dwSeedResultTextSize = (pSeedFormat->dwSizeCount * 1024) + pSeedFormat->dwExtraSize + pSeedFormat->dwSeedHeaderSize;
...
BOOL WriteOuputFile(PFILEVIEW pFileView, PSEEDFORMAT pSeedFormat, LPCTSTR lpOutput_Filename)
{
    ...
    if (!WriteFile(hOutputFile, pSeedFormat->pBuffer, pSeedFormat->dwSeedResultTextSize, &written, NULL))
    {
        printf("[!] It's not found the WriteFile!!\n");
        return FALSE;
    }
...
void ClearHandle(PFILEVIEW pFileView, PSEEDFORMAT pSeedFormat)
{
    // Clear memory
    UnmapViewOfFile(pFileView->pBaseAddr);
    VirtualFree(pSeedFormat->pBuffer, 0, MEM_RELEASE);
    free(pSeedFormat->pSeedHeader);
    free(pFileView);
    free(pSeedFormat);
}
...
int main(int argc, char **argv)
{
    ...
    BYTE szEnc_OutputFile[256], szDec_OutputFile[256] = {0};

    BYTE pbszUserKey[16] = {0x0F, 0x031, 0x045, 0x067, 0x089, 0x0AB, 0x0CD, 0x0EF, 0x0CE, 0x034, 0x056, 0x068, 0x093, 0x0BC, 0x0DE, 0x0F7};
    BYTE pbszIV[16] = {0x04E, 0x0EC, 0x0AA, 0x088, 0x066, 0x044, 0x022, 0x011, 0x0DE, 0x0EC, 0x0AA, 0x098, 0x076, 0x054, 0x032, 0x010};

    LPTSTR szSeedFilePath = (LPTSTR)calloc(20, sizeof(LPTSTR));
    snprintf(szSeedFilePath, strlen(argv[2]) + 1, argv[2]);

    sprintf(szDec_OutputFile, ".\\Dec_%s", argv[2]);
    sprintf(szEnc_OutputFile, ".\\Enc_%s", argv[2]);

    ///////////////////////////////////
    // Read the file
    printf("[+] Get the file PE view.\n");
    if (!(pFileView = GetFileView(szSeedFilePath)))
    {
    ...
    {
        ///////////////////////////////////
        // Encrypt the seed file
        printf("[+] Encrypt the seed file.\n");
        if (!(pSeedFormat = Encrypt_Seed(pFileView, pbszUserKey, pbszIV)))
        {
            printf("[!] It's not found the pSeedFormat !!\n");
            return FALSE;
        }
        ...
        if (!WriteOuputFile(pFileView, pSeedFormat, szEnc_OutputFile))
        {
            printf("[!] It's not found the output File !!\n");
            return FALSE;
        }
    }
    else if (!strcmp(argv[1], "-d"))
    {
        ...
        printf("[+] Decrypt the seed file.\n");
        if (!(pSeedFormat = Decrypt_Seed(pFileView, pbszUserKey, pbszIV)))
        {
            printf("[!] It's not found the pSeedFormat !!\n");
            return FALSE;
        }
        ...
        // Write the file
        printf("[+] Write the output file.\n");
        if (!WriteOuputFile(pFileView, pSeedFormat, szDec_OutputFile))
        {
            printf("[!] It's not found the output File !!\n");
            return FALSE;
        }
    ...
    ClearHandle(pFileView, pSeedFormat);
